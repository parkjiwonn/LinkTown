          ...
          <div class="hidden md:block">
            <div class="flex items-baseline ml-10 space-x-4">
              <div class="flex flex-row">
                ...
                <div class="flex flex-row">
                  <div class="w-3 h-3 rounded-full my-auto mr-3 ml-3"
                ...
                <div class="items-center" @mouseover="showSubMenu = true" @mouseleave="showSubMenu = false">
                  <img @click="goToMyPage" :src="profileImage" alt="person" class="w-8 h-8 rounded-full my-auto ml-5" />

                  <div v-if="showSubMenu" class="absolute bg-white shadow-md mt-2 rounded z-50">
                    <!-- 여기에 소메뉴 내용 -->
                    <router-link to="/Mypage" class="block px-4 py-2 text-gray-800 hover:bg-gray-100">마이페이지</router-link>
                    <div @click="logout" class="block px-4 py-2 text-gray-800 hover:bg-gray-100 cursor-pointer">
                      로그아웃
                    </div>
                  </div>
                </div>
...
<script>
import { GetUserLoginInfo } from "../../../Util/GetUserInfo.js";
import {
  getUserInfo,
  removeCookies,
  removeLocalStorage,
} from "../../../Util/GetUserInfo.js";
...
export default {
  ...
  async mounted() {
    const processedHeaderInfo = await GetUserLoginInfo(this.headerInfo);
    ...
    if (authResult.method === "autoLogin") {
      ...
      console.log(
        "mounted() in HeaderAfterLogin authResult.method:",
        authResult.method
      );
